...
void main()
{
	...
	vec4 projectorColor = texture2D(projtex, projector_uv.xy);
 
	booleanSample.x = pass.position_clip_alt.z + 1.0;
	projectorColor *= texture(tex_bool, booleanSample)
	...
	fragColor = (diffuseColor * kd + specularColor * ks) * lightColor * projectorColor + AMBIENT;
}
