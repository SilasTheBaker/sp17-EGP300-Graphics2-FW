...
{
	vec4 lerp1 = lerp(p0, p1, t);
	vec4 lerp2 = lerp(p1, p2, t);
...
vec4 sampleBezier3(in vec4 p0, in vec4 p1, in vec4 p2, in vec4 p3, const float t)
{
	vec4 lerp1 = lerp(p0, p1, t);
	vec4 lerp2 = lerp(p1, p2, t);	
	vec4 lerp3 = lerp(p2, p3, t);

	return lerp(lerp(lerp1, lerp2, t), lerp(lerp2, lerp3, t), t);
}
...
void main()
{
	...
	//passColor = vec4(1.0f, 0.0f, 0.0f, 1.0f);
	//gl_Position = mvp * vec4(100, 200, 0, 1);
	//EmitVertex();
